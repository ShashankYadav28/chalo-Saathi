...
struct SearchRideView: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        ...
                        VStack(alignment: .leading, spacing: 0) {
                            ...
                            .disabled(
                                viewModel.isLoading ||
                                fromLocationSearchCompleter.searchQuery.isEmpty ||
                                toLocationSearchCompleter.searchQuery.isEmpty
                            )
                            .opacity(
                                (viewModel.isLoading ||
                                 fromLocationSearchCompleter.searchQuery.isEmpty ||
                                 toLocationSearchCompleter.searchQuery.isEmpty) ? 0.6 : 1.0
                            )
                        }
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .background(
                        ...
                        VStack(spacing: 16) {
                            ...
                            HStack(spacing: 16) {
                                ...
                                StatCard(icon: "indianrupeesign.circle.fill",
                                         title: "Saved",
                                         value: "₹0",
                                         color: .green)
                            }
            ...
            .alert("No Rides Found", isPresented: $showNoRideAlert) {
                ...
                Text("No rides were found for your selected route and date.")
            }
            
            // FROM suggestions
            Group {
                if activeSearchField == .from,
                   !fromLocationSearchCompleter.searchresult.isEmpty,
                   let anchor = fromAnchor {
                    GeometryReader { proxy in
                        let rect = proxy[anchor]
                        suggestionCard(
                            for: .from,
                            completions: fromLocationSearchCompleter.searchresult,
                            anchorRect: rect
                        )
                        .position(
            ...
            Group {
                if activeSearchField == .to,
                   !toLocationSearchCompleter.searchresult.isEmpty,
                   let anchor = toAnchor {
                    GeometryReader { proxy in
                        let rect = proxy[anchor]
                        suggestionCard(
                            for: .to,
                            completions: toLocationSearchCompleter.searchresult,
                            anchorRect: rect
                        )
                        .position(
                            x: UIScreen.main.bounds.width / 2,
    ...
    private func performSearch() {
        let searchFrom = fromLocationSearchCompleter.searchQuery.isEmpty
            ? locationManager.currentAddress
            : fromLocationSearchCompleter.searchQuery
        let searchTo = toLocationSearchCompleter.searchQuery
        
        // Decide coordinates to use:
        var pickupCoord = fromCoordinate ?? locationManager.currentLocation
        var dropCoord   = toCoordinate
        
        // Helper to actually call the ViewModel once we have coords (or nil)
        func runSearch() {
        ...
        // If we are missing drop coordinate, geocode the "to" address first
        if dropCoord == nil {
            locationManager.getCoordinates(from: searchTo) { coord in
                DispatchQueue.main.async {
                    ...
                    // Even if geocoding fails, run search (without radius filter)
                    runSearch()
                }
            }
        } else {
