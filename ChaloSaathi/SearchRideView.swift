...
// MARK: - Anchor preference keys (names match your usage in the view)
...
struct SearchRideView: View {
    ...
    var body: some View {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        ...
                        VStack(spacing: 0) {
                            HStack(spacing: 12) {
                                ...
                                TextField("Enter the Location", text: $fromLocationSearchCompleter.searchQuery)
                                    ...
                                    .anchorPreference(key: FromFieldAnchorKey.self, value: .bounds) { anchor in anchor }
                            }
                            .padding(.vertical, 12)
                            .padding(.horizontal)
                            .background(RoundedRectangle(cornerRadius: 12).fill(Color.white).shadow(color: Color.black.opacity(0.1), radius: 4, x: 0, y: 2))
                        }
                        ...
                        VStack(spacing: 0) {
                            HStack(spacing: 12) {
                                ...
                                TextField("Search Destination", text: $toLocationSearchCompleter.searchQuery)
                                    ...
                                    .anchorPreference(key: ToFieldAnchorKey.self, value: .bounds) { anchor in anchor }
                            }
                            .padding(.vertical, 12)
                            .padding(.horizontal)
                            .background(RoundedRectangle(cornerRadius: 12).fill(Color.white).shadow(color: Color.black.opacity(0.1), radius: 4, x: 0, y: 2))
                        ...
                        VStack(alignment: .leading, spacing: 0) {
                            HStack {
                                Image(systemName: "clock").font(.system(size: 20)).foregroundColor(.primary)
                                Text("TIME:").font(.system(size: 16, weight: .regular)).foregroundColor(.primary)
                                Spacer()
                                DatePicker("", selection: $selectedDateTime, displayedComponents: [.date, .hourAndMinute]).labelsHidden().disabled(searchAnyDate).opacity(searchAnyDate ? 0.5 : 1.0)
                            }
                            .padding(.vertical, 12)
                            .padding(.horizontal)

                            Toggle("Search rides on any date", isOn: $searchAnyDate).padding(.horizontal).padding(.bottom, 8)

                            Button(action: { dismissSuggestions(); performSearch() }) {
                                HStack {
                                    if viewModel.isLoading { ProgressView().tint(.white) }
                                    else { Image(systemName: "magnifyingglass").font(.system(size: 18, weight: .bold)).foregroundColor(.white) }
                        ...
                        // Recent activity card (same as before)
                        VStack(spacing: 16) {
                ...
                if activeSearchField == .from, !fromLocationSearchCompleter.searchresult.isEmpty, let anchor = fromAnchor {
                    GeometryReader { proxy in
                        ...
                        suggestionCard(for: .from, completions: fromLocationSearchCompleter.searchresult, anchorRect: rect)
                            .position(x: UIScreen.main.bounds.width / 2, y: computeCardY(for: rect, proxy: proxy, completions: fromLocationSearchCompleter.searchresult))
                    }.coordinateSpace(name: "root")
                }
                ...
                if activeSearchField == .to, !toLocationSearchCompleter.searchresult.isEmpty, let anchor = toAnchor {
                    GeometryReader { proxy in
                        ...
                        suggestionCard(for: .to, completions: toLocationSearchCompleter.searchresult, anchorRect: rect)
                            .position(x: UIScreen.main.bounds.width / 2, y: computeCardY(for: rect, proxy: proxy, completions: toLocationSearchCompleter.searchresult))
                    }.coordinateSpace(name: "root")
                }
        ...
        .onAppear {
            ...
            // default pickup coordinate = current location if available
    ...
    // Suggestion card builder (same UI as you had)
    ...
    private func performSearch() {
        ...
        // Decide coordinates to use:
        var pickupCoord = fromCoordinate ?? locationManager.location
        var dropCoord   = toCoordinate
        ...
        // If drop coord missing, geocode to-address, then search
        if dropCoord == nil {
            locationManager.getCoordinates(from: searchTo) { coord in
                DispatchQueue.main.async {
                    if let coord = coord {
                        dropCoord = coord
                        toCoordinate = coord
